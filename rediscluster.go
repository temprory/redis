...
func (rds *RedisCluster) LoadScript(tag string, src string) error {
	...
	if err != nil {
		logError("redis ScriptLoad '%s' failed: %v", tag, err)
	}
	return err
}
...
func (rds *RedisCluster) reLoadScript(tag string) error {
	...
	if err != nil {
		logError("redis ReLoadScript '%s' failed: %v", tag, err)
	}
...
func NewRedisCluster(conf RedisClusterConf) *RedisCluster {
	...
	logInfo("db.NewRedisCluster Connect To Redis ...")
	...
	cmd := client.Ping()
	if cmd.Err() != nil {
		logFatal("db.NewRedisCluster client.Ping() Failed: %v", cmd.Err())
	}
	...
	logInfo("db.NewRedisCluster Connect To Redis Success")
